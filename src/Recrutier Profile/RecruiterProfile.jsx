...
const RecruiterProfile = () => {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            // Prepare data - Flatten the structure instead of nested object
            const updateData = {
                ...
                userType: 'recruiter' // Add user type
            }
            ...
            setTimeout(() => {
                console.log("Navigating to /companyInfo")
            ...
            <div className='h-full flex flex-col gap-6 items-center justify-center px-4 py-6'>
                ...
                <div className='w-full max-w-2xl'>
                    <div className='flex flex-row justify-between items-center relative'>
                        ...
                        <div className='flex flex-col gap-2 items-center relative z-10 bg-white px-2'>
                            ...
                            <p className='text-xs font-medium text-gray-500'>Company Details</p>
                        </div>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className='rounded-lg border border-gray-200 p-5 flex flex-col gap-6 bg-white w-full max-w-2xl'>
                    ...
                    <div className='flex flex-col gap-1 w-full text-start'>
                        <p className='text-[17px] font-semibold text-black'>Recruiter Information</p>
                    ...
                    <div className='space-y-5'>
                        <div>
                            ...
                            <input type="text" {...register('fullName', { required: "Full name is required", minLength: { value: 2, message: "Name must be at least 2 characters" } })} placeholder="e.g., John Doe" className='w-full px-4 py-2.5 bg-[#FAFBFC] border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#E94560] focus:border-transparent' />
                            {errors.fullName && <p className='text-red-500 text-xs mt-1'>{errors.fullName.message}</p>}
                        </div>

                        <div>
                            <label className='block text-gray-700 font-medium mb-2'>Phone Number *</label>
                            <input type="tel" {...register('phoneNumber', { required: "Phone number is required", pattern: { value: /^[0-9+\-\s()]{10,15}$/, message: "Please enter a valid phone number" } })} placeholder="e.g., +1 234 567 8900" className='w-full px-4 py-2.5 bg-[#FAFBFC] border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#E94560] focus:border-transparent' />
                            {errors.phoneNumber && <p className='text-red-500 text-xs mt-1'>{errors.phoneNumber.message}</p>}
                        </div>

                        <div>
                            <label className='block text-gray-700 font-medium mb-2'>Position / Job Title *</label>
                            <input type="text" {...register('position', { required: "Position is required", minLength: { value: 2, message: "Position must be at least 2 characters" } })} placeholder="e.g., HR Manager, Talent Acquisition Specialist" className='w-full px-4 py-2.5 bg-[#FAFBFC] border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#E94560] focus:border-transparent' />
